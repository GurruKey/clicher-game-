...
def main() -> None:
    ...
    root.attributes("-alpha", 0.0) # Start transparent to prevent UI flashing
    ...
    root.update() # Process geometry changes before showing loading overlay
    ...
    def handle_exit():
        root.destroy()
        # Clear all __pycache__ folders recursively
        for cache in current_dir.rglob("__pycache__"):
            shutil.rmtree(cache, ignore_errors=True)
        sys.exit(0)

    # Handle standard window close signal
    ...
    ui = UIState(root, handlers={
        ...
        "items_info": lambda: show_view("items", view_builders.build_items_view),
        "items_editor": lambda: show_view("items_editor", view_builders.build_item_editor_view, True),
        ...
        "types": lambda: show_view("types", view_builders.build_types_view),
        "types_info": lambda: show_view("types", view_builders.build_types_view),
        
        "locations": lambda: show_view("locations", view_builders.build_locations_view),
        "locations_info": lambda: show_view("locations", view_builders.build_locations_view),
        ...
        "map_info": lambda: show_view("map", view_builders.build_map_view),
        
        "help": lambda: show_view("help", view_builders.build_help_view),
        "help_info": lambda: show_view("help", view_builders.build_help_view),
    ...
    # Show loading overlay before making window visible
    loading.show_loading()
    root.attributes("-alpha", 1.0) # Make window opaque now that loading is shown
