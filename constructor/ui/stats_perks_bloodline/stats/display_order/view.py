...
def create_stats_config_view(
    ...
    def render_categories():
        ...
        for c_idx, cat in enumerate(local_config):
            ...
            ModernButton(title_bar, text="Remove", command=lambda i=c_idx: remove_cat(i), bg="#3a1a1a", fg="red", bd=0, padx=4).pack(side="right")
            ModernButton(title_bar, text="Down", command=lambda i=c_idx: move_cat(i, 1), bd=0, padx=4).pack(side="right", padx=2)
            ModernButton(title_bar, text="Up", command=lambda i=c_idx: move_cat(i, -1), bd=0, padx=4).pack(side="right", padx=2)
            ...
            if not stats_list:
                ...
                for s_idx, sid in enumerate(stats_list):
                    ...
                    ModernButton(s_row, text="Down", command=lambda ci=c_idx, si=s_idx: move_stat(ci, si, 1), bg="#1a1a1a", bd=0, padx=4).pack(side="right", padx=2)
                    ModernButton(s_row, text="Up", command=lambda ci=c_idx, si=s_idx: move_stat(ci, si, -1), bg="#1a1a1a", bd=0, padx=4).pack(side="right", padx=2)
