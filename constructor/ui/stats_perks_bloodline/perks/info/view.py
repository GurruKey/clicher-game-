...
def create_perks_info_view(
    ...
    def render_list():
        ...
        for perk in sorted(current_perks, key=lambda x: x["name"].lower()):
            ...
            lbl = tk.Label(row_inner, text=perk["name"], bg=ROW_BG); lbl.pack(side="left", fill="x", expand=True)
            widgets = [row, row_inner, lbl]; row_entries.append({"id": perk["id"], "widgets": widgets})
