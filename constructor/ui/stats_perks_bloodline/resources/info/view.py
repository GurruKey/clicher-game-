...
def create_resources_info_view(
    ...
    # --- Search & List ---
    search_row = tk.Frame(list_view.top_frame); search_row.pack(fill="x", pady=(0, 8))
    ...
    btn_create_edit = ModernButton(list_view.bottom_frame, text="Create/Edit", bg=BUTTON_BG, fg="#b8f5b8"); btn_create_edit.pack(side="left", fill="x", expand=True, padx=(0, 5))
    btn_exit = ModernButton(list_view.bottom_frame, text="Exit", state="disabled"); btn_exit.pack(side="left", fill="x", expand=True, padx=(5, 0))
    ...
    def render_list():
        ...
        for res in sorted(current_resources, key=lambda x: x["label"].lower()):
            ...
            lbl = tk.Label(row_inner, text=res["label"], bg=ROW_BG); lbl.pack(side="left", fill="x", expand=True)
            row_widgets = [row, row_inner, lbl]; entry = {"id": res["id"], "widgets": row_widgets}; row_entries.append(entry)
            def bind_ev(w, r=res):
