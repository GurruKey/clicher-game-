...
def create_rarities_view(
    ...
    add_frame = tk.LabelFrame(detail_frame, text="Add/Edit rarity")
    ...
    add_row.pack(fill="x", padx=10, pady=(8, 8))

    tk.Label(add_row, text="Id").grid(row=0, column=0, sticky="w")
    tk.Entry(add_row, textvariable=add_id_var, width=15).grid(
        row=0, column=1, sticky="w", padx=(6, 12)
    )
    tk.Label(add_row, text="Label").grid(row=0, column=2, sticky="w")
    tk.Entry(add_row, textvariable=add_label_var, width=15).grid(
        row=0, column=3, sticky="w", padx=(6, 12)
    )
    tk.Label(add_row, text="Color").grid(row=0, column=4, sticky="w")
    
    color_input_frame = tk.Frame(add_row)
    color_input_frame.grid(row=0, column=5, sticky="w", padx=(6, 0))
    
    color_entry = tk.Entry(color_input_frame, textvariable=add_color_var, width=10)
    ...
    color_preview_canvas.pack(side="left", padx=(4, 0))
    color_rect = color_preview_canvas.create_rectangle(0, 0, 20, 20, fill="#ffffff", outline="")
    color_preview_canvas.bind("<Button-1>", lambda _: handle_pick_color())
    ...
    save_button = ModernButton(action_row, text="Save")
    save_button.pack(side="right")

    delete_button = ModernButton(action_row, text="Delete")
    delete_button.pack(side="right", padx=(0, 8))

    new_button = ModernButton(action_row, text="New")
    ...
    def on_text_change(*_args: object) -> None:
        # Use current editing color if available and valid
        color = selected_color["value"]
        current_edit_color = add_color_var.get().strip()
        if re.match(r"^#[0-9a-fA-F]{6}$", current_edit_color):
            color = current_edit_color
            # Update the color preview box
            try:
                color_preview_canvas.itemconfig(color_rect, fill=color)
            except Exception:
                pass
    ...
    add_color_var.trace_add("write", on_text_change)
    ...
    # Rarity List using ScrollableFrame
